...
},{"./box":3,"./test":16}],2:[function(require,module,exports){
"use strict";
...
var Neck = function () {
  ...
  _createClass(Neck, [{
    key: "display",
    value: function display(processing) {
...
var _melody = require('../scores/melody');
...
var Box = function () {
	function Box(customOptions) {
		...
		var notesToDisplay = (0, _notes2.default)(_melody2.default);

		// Parts
		this.body = new _body2.default(options);
		this.comb = new _comb2.default(options, notesToDisplay);
		this.cylinder = new _cylinder2.default(options, notesToDisplay);

		// Music & animation
		this.synthesizer = new _synthesizer2.default();
		this.sequencer = new _sequencer2.default();
		this.sequencer.load(_melody2.default);
		this.sequencer.onNoteEvent(function (note) {
			_this.comb.triggerNoteAnimation(note.n);
			_this.cylinder.triggerNoteAnimation(note.n);
			_this.synthesizer.playNote(note.n, 1, 0.5);
		});

		_lodash2.default.assign(options, customOptions);
		...
		value: function setup() {
			...
			return function (processing) {
				...
				processing.draw = function () {
					...
					_this2.body.display(processing);
					_this2.cylinder.display(processing);
					_this2.comb.display(processing);
...
},{"../comb":5,"../cylinder":8,"../scores/melody":10,"../sequencer":11,"../synthesizer":14,"./body":2,"./notes":4,"lodash":20}],4:[function(require,module,exports){
'use strict';
...
var noteWeight = function noteWeight(note) {
	return note.length + order.indexOf(note[0]) * 2 + Number(note[note.length - 1]) * order.length * 3;
};

var gatherNotesFromScore = function gatherNotesFromScore(score) {
	var notesSet = {};
	...
	return Object.keys(notesSet).sort(function (a, b) {
		return noteWeight(a) - noteWeight(b);
	});
};

exports.default = gatherNotesFromScore;
...
var Comb = function () {
	...
	_createClass(Comb, [{
		key: 'setupTeeth',
		value: function setupTeeth(options, notes) {
			var teeth = [];

			notes.forEach(function (note, index) {
				teeth.push(new _tooth2.default(options, index * options.tooth.width, note));
			});
		...
		key: 'displayTeeth',
		value: function displayTeeth(processing) {
			this.teeth.forEach(function (tooth) {
				tooth.display(processing);
			});
		}
	}, {
		key: 'triggerNoteAnimation',
		value: function triggerNoteAnimation(note) {
			var teethToAnimate = this.teeth.filter(function (tooth) {
				return tooth.note === note;
			});
			teethToAnimate.forEach(function (tooth) {
...
},{"./tooth":7}],6:[function(require,module,exports){
"use strict";
...
var Oscillator = function () {
	...
	_createClass(Oscillator, [{
		...
		value: function computePosition(t) {
			...
			return Math.exp(-this.options.oscillator.gamma * t) * this.options.oscillator.amplitude * Math.cos(this.options.oscillator.omega1 * t);
		}
	}]);
...
var vibrationDecay = 500;

var Tooth = function () {
	function Tooth(options, offsetX, note) {
		_classCallCheck(this, Tooth);

		this.note = note;
		this.oscillator = new _oscillator2.default(options);
	...
	_createClass(Tooth, [{
		...
		key: 'displayTooth',
		value: function displayTooth(processing) {
			var absZ = Math.abs(this.z) / 5;
			var y1 = this.length / 4 + absZ;
			var y2 = this.length / 1.5 + absZ;
			var y3 = this.length + absZ;
		...
		value: function triggerAnimation() {
			...
			window.setTimeout(function () {
				_this.oscillator.triggerVibration();
				_this.twisted = false;
				_this.z = 0;
...
},{"./oscillator":6}],8:[function(require,module,exports){
'use strict';
...
var Cylinder = function () {
	function Cylinder(options, notes) {
	...
	_createClass(Cylinder, [{
		...
		key: 'triggerNoteAnimation',
		value: function triggerNoteAnimation(note) {
			var pinPositionX = this.notes.indexOf(note) * this.options.tooth.width + 0.5 * this.options.tooth.width;
			this.pinsBuffer.push(new _pin2.default(this.options, pinPositionX));
		}
	}, {
		key: 'displayPinsBuffer',
		value: function displayPinsBuffer(processing) {
			var _this = this;

			this.pinsBuffer.forEach(function (pin) {
				pin.angle += _this.options.cylinder.rotationSpeed;
				pin.display(processing);
			});
			// Filter notes that are no longer visible
			this.pinsBuffer = this.pinsBuffer.filter(function (pin) {
...
},{"./pin":9}],9:[function(require,module,exports){
"use strict";
...
var Pin = function () {
	function Pin(options, positionX) {
	...
	_createClass(Pin, [{
		...
		value: function display(processing) {
			...
			processing.ellipse(this.x, 155 - factor, 2 * Math.abs(processing.sin(this.angle)) + this.height, 2 * Math.abs(processing.sin(this.angle)) + this.height);
		}
	}]);
...
var Sequencer = function () {
	...
	_createClass(Sequencer, [{
		...
		value: function playNextNote() {
			...
			var note = this.score[this.scoreIndex];

			window.setTimeout(function () {
				if (_this.playing) {
					_this.dispatchNoteEvent(note);
					_this.playNextNote();
				}
			}, note.t * this.tic);
		...
		key: "dispatchNoteEvent",
		value: function dispatchNoteEvent(note) {
			this.noteEventCallbacks.forEach(function (cb) {
				cb(note);
			});
		}
	}, {
		key: "onNoteEvent",
		value: function onNoteEvent(cb) {
...
var _frequencies2 = _interopRequireDefault(_frequencies);
...
var Synthesizer = function () {
	function Synthesizer() {
		...
		this.destination = _audioContext2.default.createGain();
		this.destination.gain.value = 1;
		this.destination.connect(_audioContext2.default.destination);
	...
	_createClass(Synthesizer, [{
		key: 'playNote',
		value: function playNote(name, duration, offset) {
			if (this.activeVoices[name]) {
				this.activeVoices[name].osc.stop(_audioContext2.default.currentTime + offset);
				delete this.activeVoices[name];
			}

			var voice = new _voice2.default(_frequencies2.default[name]);
			this.activeVoices[name] = voice;

			voice.gainNode.connect(this.destination);
			voice.gainNode.gain.cancelScheduledValues(0);
			// Time in seconds
			voice.gainNode.gain.setValueAtTime(0.25, _audioContext2.default.currentTime + offset);
			voice.gainNode.gain.linearRampToValueAtTime(0, _audioContext2.default.currentTime + duration + offset);
		}
	}]);
...
},{"./audio-context":12,"./frequencies":13,"./voice":15}],15:[function(require,module,exports){
'use strict';
...
var Voice = function Voice(frequency) {
...
},{"./audio-context":12}],16:[function(require,module,exports){
'use strict';
...
var Test = function Test() {
	...
	this.draw = (0, _setup2.default)(new _tooth2.default(), new _synthesizer2.default());
};

exports.default = Test;

},{"../synthesizer":14,"./setup":18,"./tooth":19}],17:[function(require,module,exports){
"use strict";
...
exports.default = function (tooth, synthesizer) {
	return function (processing) {
		processing.setup = function () {
...
var Tooth = function () {
	...
	_createClass(Tooth, [{
		...
		key: 'mousePressed',
		value: function mousePressed(processing) {
		...
		key: 'mouseDragged',
		value: function mouseDragged(processing) {
...
},{"./oscillator":17}],20:[function(require,module,exports){
(function (global){
/**
 * @license
